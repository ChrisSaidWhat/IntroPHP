<?php
    ...
    try {
        ...
        $sql = "SELECT events_id, events_name, events_description, events_presenter, DATE_FORMAT(events_date, '%W %M %e %Y') AS eventsFormatDate, TIME_FORMAT(events_time, '%h:%i %p') AS eventsFormatTime, DATE_FORMAT(events_date_inserted, '%W %M %e %Y') AS eventsInsertedFormatDate, DATE_FORMAT(events_date_updated, '%W %M %e %Y') AS eventsUpdatedFormatDate FROM wdv341_events WHERE events_id = :eventID";
...
<html lang="en">
    <head>
        ...
        <title>Web Development Event</title>
    ...
    <body>
        <h1>Web Development Single Event Schedule</h1>
